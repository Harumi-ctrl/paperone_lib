...
//   return docs;
// });
final lifestyleItemsProviderTwo = StreamProvider<QuerySnapshot>((ref) {
...
class Lifestyle extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final foodsItemsAsyncValue = ref.watch(lifestyleItemsProviderTwo);
    /////
    ////
    final bookmarksAsyncValue = ref.watch(bookmarksProvider);
    //
    //
    final isSubscribedIOSlifestyle = ref.watch(inAppPurchaseManagerProvider);
    //
    //
    //
    ...
    return Scaffold(
      ...
      onRefresh: () async {
        ref.refresh(lifestyleItemsProviderTwo);
      },
      child: foodsItemsAsyncValue.when(
        data: (snapshot) {
          //
          //
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              //
              bool isBookmarked =
                  bookmarkedItems.any((bookmark) => bookmark['title'] == title);
              String? bookmarkId;
              if (isBookmarked) {
...
//特定アカウントの場合
                                  ...
                                  if (isSubscribedIOSlifestyle.isSubscribed ==
                                      true) {
                                    if (isBookmarked && bookmarkId != null) {
                                      ref
                                          .read(firestoreServiceProvider)
                                          .removeBookmark(user.uid, bookmarkId);
                                    } else {
                                      ref
                                          .read(firestoreServiceProvider)
                                          .addBookmark(user.uid, item);
                                    }
                                  } else if (isSubscribedIOSlifestyle
                                          .isSubscribed ==
                                      false) {
                                    showDialog(
                                        context: context,
